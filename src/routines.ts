...
export function replaceAll(
  ...
  while (currentOffset < buffer.length) {
    ...
    if (chunkQueued.buffer) {
      chunkString =
        chunkQueued.buffer.toString("binary") + chunk.toString("binary");
    } else chunkString = chunk.toString("binary");
    ...
    while ((match = find.exec(chunkString)) !== null) {
      result.push({
        ...
        offset:
          currentOffset +
          (match.index - (chunkQueued.buffer ? chunkQueued.buffer.length : 0)),
      });
      chunkString = [
    ...
    if (chunkQueued.buffer) {
      ...
      chunk = Buffer.from(
        chunkString.slice(chunkQueued.buffer.length),
        "binary",
      );
    } else chunk = Buffer.from(chunkString, "binary");
    ...
    if (chunkQueued.buffer) currentOffset += chunkQueued.buffer.length;
  }
