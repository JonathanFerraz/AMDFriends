...
const _tempDir = mkdtempSync(resolve(tmpdir(), "friendlyamd-"));

export async function patchFile(
  filePath: string,
  options: PatchOptions,
): Promise<PatchingResult | null> {
  const result: PatchingResult = {
    patchedPath: resolve(
      dirname(filePath),
      basename(filePath, extname(filePath)) + ".patched" + extname(filePath),
    ),
    patchedRoutines: [],
  };

  if (options.inPlace) result.patchedPath = filePath;
  ...
  try {
    ...
    else console.error(`Error while opening ${filePath}: ${err.message}`);

    return null;
  }

  result.patchedRoutines.push(
    ...replaceAll(
      buffer,
  ...
  if (result.patchedRoutines.length) {
    if (!options.dryRun) {
      ...
      const _tempFile = resolve(
        _tempDir,
        md5(filePath + Date.now().toString()),
      );
      await writeFile(_tempFile, buffer);

      if (options.clearXA) await clearXAFile(_tempFile);

      if (options.sign) await signFile(_tempFile);
...
export async function signFile(filePath: string) {
  return await spawnProcess("/usr/bin/codesign", [
    "--force",
    "--sign",
    "-",
    filePath,
